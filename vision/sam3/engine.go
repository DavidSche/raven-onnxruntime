...
func NewEngine(cfg Config) (*Engine, error) {
	...
	visSession, err := oc.OnnxEngine.NewSession(cfg.VisionModelPath, oc.SessionOptions)
	if err != nil {
		oc.Destroy()
		return nil, fmt.Errorf("failed to create vision encoder ONNX session: %w", err)
	}

	txtSession, err := oc.OnnxEngine.NewSession(cfg.TextModelPath, oc.SessionOptions)
	if err != nil {
		visSession.Destroy()
		oc.Destroy()
