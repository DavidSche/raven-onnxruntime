...
// DetEngine is the EdgeCrafter detection engine.
...
// NewDetEngine creates a new EdgeCrafter detection engine.
func NewDetEngine(cfg Config) (*DetEngine, error) {
	ortlog.Infow("creating EdgeCrafter detection engine",
	...
	ortlog.Infow("EdgeCrafter detection engine created successfully",
		"modelPath", cfg.ModelPath,
...
func (e *DetEngine) Destroy() {
	if e.session != nil {
		ortlog.Infow("destroying EdgeCrafter detection engine", "modelPath", e.config.ModelPath)
		e.session.Destroy()
	}
}

// Predict runs detection on a single image.
func (e *DetEngine) Predict(img image.Image) ([]DetResult, error) {
	...
	// Resolve outputs: pred_logits and pred_boxes
	var logitsName, boxesName string
	...
	if boxesName == "" || logitsName == "" {
		...
		return nil, fmt.Errorf("EdgeCrafter det model requires pred_boxes and pred_logits outputs, got %v", e.session.OutputNames)
	}
	...
	if !ok0 || !ok1 || boxesOut == nil || logitsOut == nil {
		...
		return nil, fmt.Errorf("EdgeCrafter det model output %q or %q does not exist", boxesName, logitsName)
	}
	...
	results := e.postprocess(boxesData, logitsData, numDetections, numClasses, params)
...
// PredictBatch runs detection on a batch of images.
...
func (e *DetEngine) logPredictTimings(batchSize int, preprocessElapsed, runElapsed, postprocessElapsed, totalElapsed time.Duration) {
	...
	ortlog.Infow("edgecrafter det timings",
