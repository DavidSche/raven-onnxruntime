...
//
// 字段锁归属（跨锁字段审计结论，见项目规范 §17）：isDestroyed 与 imageEmbeddings
// 的全部访问经 mu——Destroy 与 DecodeRaw 可能并发，无锁时"检查通过后 Value 被并发
// 销毁"会构成 use-after-free / double-free（与 raven-go simulation 的 stopped 同类）。
...
// Destroy releases the image feature cache（幂等，持锁）
